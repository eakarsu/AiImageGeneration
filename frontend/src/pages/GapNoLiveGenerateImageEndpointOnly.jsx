...
// Auto-generated page for: No live /generate-image endpoint (only request CRUD)

export default function GapNoLiveGenerateImageEndpointOnly() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-live-generate-image-endpoint-only', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No live /generate-image endpoint (only request CRUD)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No live /generate-image endpoint (only request CRUD)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No live /generate-image endpoint (only request CRUD)'}
        </button>
      </form>
